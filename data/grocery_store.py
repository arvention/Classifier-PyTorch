...
class GroceryStore(Dataset):
    ...
    def __init__(self,
        ...
        Arguments:
            ...
            classes {string} -- indicates if the classes to be used is
        ...
        self.image_path = osp.join(self.data_path, 'dataset', '%s')

        list_path = ''
        if self.mode == 'train':
            list_path = osp.join(self.data_path, 'dataset', 'train.txt')
        elif self.mode == 'val':
            list_path = osp.join(self.data_path, 'dataset', 'val.txt')
        elif self.mode == 'test':
            list_path = osp.join(self.data_path, 'dataset', 'test.txt')

        class_index = -1
        if self.classes == 'fine-grained':
            class_index = 1
        elif self.classes == 'coarse-grained':
            class_index = 2
        ...
        with open(list_path) as f:
            for line in f:
                ...
                self.targets.append(values[class_index].strip())
